...
const val SEARCH_ACCOMODATION: String = """
   SELECT 
    A.accomodation_id, A.accomodation_name, A.host_name, A.discription, A.price, 
   ...
   WHERE R.check_in < :check_in and :check_out < R.check_out  
   AND A.maximum_occupancy > :total_people 
...
const val FIND_ACCOMODATION: String = """
    SELECT A.accomodation_id, A.accomodation_name, A.host_name, A.discription, A.price, 
    A.maximum_occupancy, A.average_point, A.bed_room, A.bath_room, A.bed_count, A.option, 
